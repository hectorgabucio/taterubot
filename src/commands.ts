...
const createNewChunk = (SessionID: string) => {
  const pathToFile =
    __dirname + `/../recordings/${SessionID}/${Date.now()}.pcm`;
...
export const enter = async function (
  ...
  if (typeof activeGuildRecorders[guildId] === "undefined") {
    ...
    console.log(
      `Sliding into ${voiceChannel.name} ..., last`,
      process.hrtime(start)
    );
    ...
    voiceChannel
      ...
      .then((conn) => {
        ...
        const dispatcher = conn.play(
          __dirname + "/../sounds/taterubot-start-recording.mp3"
        );
...
export const exit = function (voice: VoiceState, channel: TextChannel) {
  ...
  dispatcher.on("finish", () => {
    ...
    fs.writeFile(
      __dirname + `/../recordings/${resolveSessionId}.json`,
      data,
      "utf8",
      (err) => {
        if (err) {
        ...
        transcoderChild.on("exit", function (code, signal) {
          console.log(
            "Transcoder process exited with " +
    ...
    console.log(
      "Destroying guild lock for",
      voiceSessionMap[voiceChannel.id].guildId
    );
